          ...
          <div class="ibar_login_box status_login">
            <div class="avatar_box">
              <p class="avatar_imgbox">
                <img src="{{ asset('home/images/' . session('photo')) }}" />
              </p>
              <ul class="user_info">
...
<script>
  ...
  $('#LikBasket').click(function(){
      var str = $(this).html();
      if (str == '点击收藏') {
          $.post("/collect", {uid : "{{ $good->users_id }}", gid : "{{ session('userid') }}" }, function(data){
              if (data == '1'){
            	  $('#LikBasket').html('取消收藏');
              }
          });
      } else {
    	  $.post("/cancelcollect", {uid : "{{ $good->users_id }}", gid : "{{ session('userid') }}" }, function(data){
              if (data == '1'){
            	  $('#LikBasket').html('点击收藏');
              }
          });
